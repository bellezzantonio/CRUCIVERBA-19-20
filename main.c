...
struct suggerimenti_v{
  ...
  struct suggerimenti_v *next;
    ...
    struct risposte_utente_v *r_u_v=NULL;
    struct risposte_verticali *r_v;
    r_v=head;
    int n, r_u, r_s, n_r;
    char c[R];
    ...
    while(r_v!=NULL){
        ...
        r_v=r_v->next;
            ...
            stampa_griglia(griglia, n, c, r_s, orientamento);
            r_v=NULL;
            break;
            }
    } 
            if(n!=n_r && r_u!=r_s){
            printf("Il numero di caratteri inserito non è valido, perfavore ricontrolla."); 
            } 
    r_u_v->next=testa;
    testa=r_u_v;  
    return r_u_v;
    ...
    struct risposte_utente_o *r_u_o=NULL;
    struct risposte_orizzontali *r_o;
    r_o=head;
    int n, r_u, r_s, n_r;
    char c[R];
    int i,orientamento=0;     //orientamento risposte 0=orizzontali
    ...
    void menu(int flag, char griglia[60][60]){
        ...
        struct risposte_orizzontali *r_f_o=NULL;  
        struct risposte_verticali *r_f_v=NULL;
        struct risposte_utente_o *r_f_u_o=NULL;  
        struct risposte_utente_v *r_f_u_v=NULL;
        int a, b, inserimento;
        int tentativi_o=3;
        int tentativi_v=3;
	    ...
	    while(1){
            ...
            case 2:
                r_u_o = r_utente_o(griglia,r_u_o, r_o);          //Funzione 6
                confronto_r_o(r_o, r_u_o);
                break;
                
            case 3:
                r_u_v = r_utente_v(griglia, r_u_v, r_v);            //Funzione 5
            ...
            case 6:
              verifica(griglia,r_o, r_v);                              //Funzione 11
            break;
